 ...
 * 
 * Draw single 5x7 pixel character at specified position.
 * Each character is defined by 7 rows of 5 bits.
 * Supports ASCII characters 32-122 (space through 'z').
 */
void drawChar(u8* framebuffer, char c, int startX, int startY, u8 r, u8 g, u8 b) {
    // 5x7 bitmap font definition (rightmost 5 bits of each byte used)
 ...
 * Characters are spaced 6 pixels apart (5 + 1), lines 9 pixels apart (7 + 2)
 */
void drawString(u8* framebuffer, const char* str, int x, int y, u8 r, u8 g, u8 b) {
    ...
    while (*str) {
        if (*str == '\n') {
            cursorX = x;
            cursorY += 9; // Move down to next line
        } else {
            drawChar(framebuffer, *str, cursorX, cursorY, r, g, b);
            cursorX += 6; // Move right to next character
        }
        str++;
    }
}
...
void drawInstructions(u8* framebuffer) {
    ...
    drawString(framebuffer, "B: Cycle mode", 10, 160, 255, 255, 255);
